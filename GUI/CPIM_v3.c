...
/* Default "energy cost" (in coupling field units) of flipping a spin */
#define FLIPPING_COST 0.01

/* Structure with the simulation data */
struct simulation {
    int lattice_configuration[X_SIZE][Y_SIZE]; /* Store lattice configuration */
    double coupling_field[X_SIZE][Y_SIZE]; /* Store coupling field */
    ...
    double flipping_cost;       /* Cost of flipping a spin */
...
/* ini_transition_probs and set_transitions_probs have to be reworked in order
to take account coupling_field dynamics */
        ...
        switch (s.lattice_configuration[random_x_coor][random_y_coor])
        {
            ...
            case 1: /* Focal point is in the up (+1) state */
                ...
                else if (spin_energy_diff < 0 || genrand64_real2() < transition_probability)
                {
                    ...
                    s.up--;
                    s.down++;
                    s.coupling_field[random_x_coor][random_y_coor] += s.flipping_cost;
                }
                break;
...
static void initialize_simulation(void)
{
    ...
    s.flipping_cost = (double)FLIPPING_COST;
